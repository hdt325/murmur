...
**Fix**: Added `_normalizeTerminalText()` to strip trailing whitespace before comparison, skip broadcast if unchanged.
...
**Fix**: Added `_lastPong` tracking, periodic ping every 10s, terminate clients silent for >60s.
...
**Fix**: Added `TTS_QUEUE_TRIM_THRESHOLD` (15), trims oldest non-playing jobs >30s when exceeded.
...
**Fix**: Removed `await` — contentUpdateCheck runs in background, doesn't block server start.
...
**Fix**: Added retry once on ECONNREFUSED/ECONNRESET with 2s delay, plus service recovery drain when Kokoro comes back online.
...
**Fix**: Already guarded by `if (micStream) return` in initMicMeter and `if (!ttsAudioCtx)` on gesture unlock. Added defensive check: clear closed AudioContext before creating new one.
...
**Root cause**: Was already fixed — entries saved via `persistWindowEntries()` (debounced 2s), loaded from `window-entries.json` on startup. Per-window map with atomic write (tmp+rename).
...
**Fix**: Added CORS middleware restricting to `localhost`/`127.0.0.1` origins only.
...
**Fix**: Already capped — wsLog capped at 200 entries (shift on push), server log entries capped at 500.
...
**Fix**: Pre-buffer blob nulled after use in onstop callback — prevents stale data on rapid re-record.
